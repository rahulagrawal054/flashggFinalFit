...
mkdir -p ./all_eras_ws_signal_StatOnly
for era in "${eras[@]}"; do
  for f in /eos/user/r/rkumarag/hgg_tth_th_cp_analysis/finalFitPreparation/outputForFinalFits_03Nov2025_Nominal/workspaces/${era}/ws_signal/*.root; do
    base=$(basename "$f" .root)
    cp -v "$f" ./all_eras_ws_signal_StatOnly/${base}_${era}.root
  done
done
...
{ time python3 RunPackager.py \
  --cats auto \
  --inputWSDir ./all_eras_ws_signal_StatOnly \
