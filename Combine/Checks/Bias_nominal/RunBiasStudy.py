...
if opts.toys:
    ...
    for ipdf,pdfName in indexNameMap.items():
        ...
        if opts.nToys > opts.split:
            ...
            toyCmd = toyCmdBase + ' -t %g -n _%s --setParameters %s=%g,%s=%.4f --freezeParameters %s --toysNoSystematics'%(opts.nToys, name, indexName, ipdf, opts.poi, opts.expectSignal, indexName)
            run(toyCmd, dry=opts.dryRun)
...
if opts.fits:
    ...
    fitCmdBase = 'combine -m %.4f -d %s -M MultiDimFit --algo singles --setParameterRanges %s=-10,30 --toysNoSystematics %s'%(opts.mH, opts.datacard, opts.poi, opts.combineOptions)
    for ipdf,pdfName in indexNameMap.items():
        name = shortName(pdfName)
        if opts.nToys > opts.split:
            for isplit in range(opts.nToys//opts.split):
                fitCmd = fitCmdBase + ' -t %g -n _%s_split%g --toysFile=%s'%(opts.split, name, isplit, toyName(name, opts.expectSignal, split=isplit))
                run(fitCmd, dry=opts.dryRun)
                system('mv higgsCombine_%s* %s'%(name, fitName(name, opts.expectSignal, split=isplit)))
            run(f'hadd {fitName(name, opts.expectSignal)}'f'BiasFits_Expected.{opts.expectSignal}/*{name}*split*.root',dry=opts.dryRun)
        else:
            fitCmd = fitCmdBase + ' -t %g -n _%s --toysFile=%s'%(opts.nToys, name, toyName(name, opts.expectSignal))
            run(fitCmd, dry=opts.dryRun)
            system('mv higgsCombine_%s* %s'%(name, fitName(name, opts.expectSignal)))

if opts.plots:
    if not path.isdir(f'BiasPlots_Expected.{opts.expectSignal}'): system(f'mkdir -p BiasPlots_Expected.{opts.expectSignal}')
    biasResults = {}
    for ipdf,pdfName in indexNameMap.items():
        name = shortName(pdfName)
        ...
        if opts.gaussianFit:
           ...
           if fitFunc:
               ...
               biasResults[name] = {"expectSignal": opts.expectSignal,"bias": mean,"sigma": sigma}
